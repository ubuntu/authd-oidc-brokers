...
func TestIsAuthenticated(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"Successfully authenticate user with password":           {firstMode: "password", token: &tokenOptions{}},

		"Authenticating with qrcode reacquires token":          {firstChallenge: "-", wantSecondCall: true, token: &tokenOptions{}},
		"Authenticating with password refreshes expired token": {firstMode: "password", token: &tokenOptions{expired: true}},
		...
		"Error when provided wrong challenge":               {firstMode: "password", token: &tokenOptions{}, firstChallenge: "wrongpassword"},
		...
		"Error when mode is password and token is invalid":                {firstMode: "password", token: &tokenOptions{invalid: true}},
		"Error when mode is password and cached token can't be refreshed": {firstMode: "password", token: &tokenOptions{expired: true, noRefreshToken: true}},
		"Error when mode is password and token refresh times out": {firstMode: "password", token: &tokenOptions{expired: true},
			customHandlers: map[string]testutils.ProviderHandler{
				"/token": testutils.HangingHandler(broker.MaxRequestDuration + 1),
			},
		},
		"Error when token is expired and can not fetch user info": {firstMode: "password", token: &tokenOptions{expired: true}, getUserInfoFails: true},
