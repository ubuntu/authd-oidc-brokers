...
func (p *Provider) getAppID(token *jwt.Token) (string, error) {
	...
	if !ok {
		return "", fmt.Errorf("failed to cast token claims to string: %v", claims["appid"])
	}
	return appID, nil
}
...
func (p *Provider) GetUserInfo(ctx context.Context, clientID string, issuerURL string, token *oauth2.Token, idToken info.Claimer, providerMetadata map[string]interface{}, deviceRegistrationData []byte) (info.User, error) {
	...
	if p.needsAccessTokenForGraphAPI && !p.skipAccessTokenForGraphAPI {
		...
		tenantID := strings.Split(strings.TrimPrefix(issuerURL, "https://login.microsoftonline.com/"), "/")[0]
...
func (p *Provider) MaybeRegisterDevice(ctx context.Context, token *oauth2.Token, username, issuerURL string, jsonData []byte) ([]byte, error) {
	...
	if len(jsonData) > 0 {
		...
		if data.DeviceID != "" && data.CertKey != nil && data.TransportKey != nil && data.AuthValue != "" && data.TPMMachineKey != nil {
			return jsonData, nil
		}
	}
	...
	return p.registerDevice(ctx, token, tenantID, domain)
}
