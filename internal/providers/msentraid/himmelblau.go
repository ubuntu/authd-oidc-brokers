...
func ensureTPMInitialized() (err error) {
	tpmInitOnce.Do(func() {
		...
		if logLevel <= log.DebugLevel {
			...
			filter += ",himmelblau=debug"
		} else if logLevel <= log.InfoLevel {
			filter += ",himmelblau=info"
		}

		msalErr := C.set_module_tracing_filter(C.CString(filter))
		if err != nil {
...
func (p *Provider) registerDevice(ctx context.Context, token *oauth2.Token, tenantID, domain string) ([]byte, error) {
	// TODO: Should we reuse an existing cert key if we have one?
