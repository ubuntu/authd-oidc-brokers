...
Log in with remote user with device authentication via GDM
    Log In With Remote User Through GDM: QR Code    ${username}    ${domain}    ${local_password}
...
Log in with remote user with local password via GDM
    Log In With Remote User Through GDM: Local Password    ${username}    ${domain}    ${local_password}
